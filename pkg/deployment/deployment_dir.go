...
func (d *Dir) Lookup(ctx context.Context, name string) (fs.Node, error) {
	...
	deployment, err := d.Client.AppsV1().Deployments(d.Namespace.Name).Get(name, metav1.GetOptions{})
	if err != nil {
		panic(err)
	}
...
func (d *Dir) ReadDirAll(ctx context.Context) ([]fuse.Dirent, error) {
	...
	deployments, err := d.Client.AppsV1().Deployments(d.Namespace.Name).List(metav1.ListOptions{})
	if err != nil {
		panic(err.Error())
	}
...
func (f *Dir) Remove(ctx context.Context, req *fuse.RemoveRequest) error {
	err := f.Client.AppsV1().Deployments(f.Namespace.Name).Delete(req.Name, &metav1.DeleteOptions{})
	if err != nil {
		return err
	}
	return nil
